...
exports.create = async (req, res) => {
    ...
    try {
        ...
        let photoprof = null
        if (req.files) {
            if ("photoprof" in req.files && req.files["photoprof"][0].filename != null) {
                photoprof = req.files["photoprof"][0].filename
            }
        }
        const salt = await bcrypt.genSalt(10)
        const password = await bcrypt.hash(plain_password, salt)
        const formateur = new Formateur(nom, prenom, email, telephone, sexe, password, photoprof, role, description, new Date(), null, id_cours)
        const formateur_id = await formateur.create()
        if (formateur_id == true) {
            res.status(201).json("Formateur créé avec succès.")
        } else if (formateur_id == 3) {
            res.status(200).json("Cet email existe déjà")
        } else {
            res.status(400).json("Erreur " + formateur_id)
        }
    } catch (error) {
...
exports.login = async (req, res, next) => {
    ...
    const formateur = new Formateur(null, null, email, null, null, password, null, null, null, null)

    try {
        const data = await formateur.authentificate()
        console.log(data)
        if (data === false) {
            res.status(200).json("Mot de passe incorrecte")
        } else if (data == 5) {
...
exports.updateNomPrenomAutre = async (req, res) => {
    const email = req.body.email
    const newnom = req.body.name
    const newprenom = req.body.prenom
    const newemail = req.body.newemail
    const newtelephone = req.body.phone
    ...
    const formateur = new Formateur(null, null, email, null, null, null, null, null, null, null)
    try {
...
exports.getOneFormateur = async (req, res) => {
    ...
    try {
        if (id) {
            console.log(id)
            const response = await formateur.seeOneFormateur(id)
            if (response) {
                res.status(200).json(response)
            } else if (response == false) {
            ...
            console.log(token.id_formateur)
            const response = await formateur.seeOneFormateur(token.id_formateur)
            if (response) {
                res.status(200).json(response)
            } else if (response == false) {
                res.status(404).json("Id non trouvé")
...
exports.getOneFormateurId = async (req, res) => {
    try {
        const id = req.body.id
        const formateur = new Formateur(null, null, null, null, null, null, null, null, null, new Date(), null, null)
        const response = await formateur.seeOneFormateur(id)
